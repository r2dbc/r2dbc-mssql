...
class RpcQueryMessageFlowUnitTests {
    ...
    void shouldTransitionFromNoneToFetching() {
        ...
        verify(this.requests).onNext(RpcQueryMessageFlow.spCursorFetch(state.cursorId, RpcQueryMessageFlow.FETCH_NEXT, 128, TransactionDescriptor.empty()));
    ...
    void shouldStopFetching() {
        ...
        verify(this.requests).onNext(RpcQueryMessageFlow.spCursorClose(state.cursorId, TransactionDescriptor.empty()));
